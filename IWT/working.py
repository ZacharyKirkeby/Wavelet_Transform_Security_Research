...
def dct_pic(nbh, nbw, height_pad, width_pad, img, blockS, seed, oldSeed):
    pixels = []
    for i in range(nbh):
        rowInd = i * blockS + height_pad * blockS
        rowInd2 = rowInd + blockS
...
'''
print("nbh: " + str(nbh) + " | nbw: " + str(nbw) + " | height_padding: " + str(height_padding) + " | width_padding: " + str(width_padding) + "\n")
string = np.array([item for sublist in string for item in sublist])
print("Minimum Coefficient (+/-): " + str(min(string)))
coeff = [abs(ele) for ele in coeff]
print("Largest Magnitude of Coefficient: " + str(max(coeff)))
print("Average Magnitude Coefficient: " + str(np.mean(coeff)))
print("Median Magnitude Coefficient: " + str(np.median(coeff)) + "\n")
print("Level: " + str(quality))
print("Largest Length: " + str(max(length)))
print("Minimum Length: " + str(min(length)))
print("Average Length: " + str(np.mean(length)))
print("Median Length: " + str(np.median(length)) + "\n")
print("Total entries: " + str(np.sum(length)))
print("Verify: " + str(len(length)) + " = " + str(nbh * nbw) + "\n")
'''
#stop timer
stopTimer = perf_counter()
print("Elapsed time during the DCT program in seconds:", stopTimer-startTimer)
...
axis[0].hist(string, bins = int(math.sqrt(len(string))))
axis[0].set_title("stream histogram")
axis[1].hist(length, bins = int(math.sqrt(len(length))))
axis[1].set_title("length histogram")
plt.show()
'''
# ---------------------------------------------------------------------------------------------------------------------#

f_path = "pepper.jpg"  # put your file path here
wavelet = 'haar'
mode = 'symmetric'
level = 1
dim1 = 512
...
cv2.imwrite("cover.bmp", np.uint8(f))
'''
...
pixels_r = [np.round(arr).tolist() for arr in OrderPixel]
...
image = idct(nbh, nbw, pixels_r, block_size, seed, oldSeed)
dct_orig = dct_pic(nbh, nbw, height_padding, width_padding, padded_img, block_size, seed, oldSeed)
cv2.imwrite("dct_orig.bmp", np.uint8(dct_orig))

#stop timer
stopTimer = perf_counter()
'''
coeffI = [abs(ele) for ele in coeffI]
stringI = np.array([item for sublist in stringI for item in sublist])
print("\nMinimum Coefficient (+/-): " + str(min(stringI)))
print("Largest Magnitude of Coefficient: " + str(max(coeffI)))
print("Average Magnitude Coefficient: " + str(np.mean(coeffI)))
print("Median Magnitude Coefficient: " + str(np.median(coeffI)) + "\n")
print("Largest Length: " + str(max(lengthI)))
print("Minimum Length: " + str(min(lengthI)))
print("Average Length: " + str(np.mean(lengthI)))
print("Median Length: " + str(np.median(lengthI)) + "\n")
print("Total entries: " + str(np.sum(lengthI)))
print("Verify: " + str(len(lengthI)) + " = " + str(nbh * nbw) + "\n")
'''
image[image > 255] = 255
image[image < 0] = 0
cv2.imwrite("idct.bmp", np.uint8(image))

print("\nElapsed time during the iDCT program in seconds:", stopTimer-startTimer)
'''
#plots
figure, axis = plt.subplots(1,2)
figure.set_size_inches(10,10)
...
#axis[0].hist(stringI, bins = int(math.sqrt(len(stringI))))
#axis[0].set_title("stream difference histogram")
used_image = img[height_padding * block_size:height_padding * block_size + nbw * block_size, 
                 width_padding * block_size:width_padding * block_size + nbw * block_size]
axis[0].hist(dct_orig.flatten() - image.flatten(), bins = int(math.sqrt(len(used_image.flatten()))))
axis[0].set_title("pixel difference histogram")
axis[1].hist(np.array(coeffI) - np.array(coeff), bins = int(math.sqrt(len(coeffI))))
axis[1].set_title("coefficient difference histogram")
plt.show()
'''
